	...
	void init(const void * _conf, void * _var, const char * _accessPassword) {
		...
		*connect_flag = 0;
		...
		if (moduleRunning) {
			...
			REMOTEXY__DEBUGLOGS.println ("\r\nModule not available, RemoteXY stopped");
		}
...
private:
	void sendPackage(uint8_t command, uint8_t *p, uint16_t length,
			uint8_t itConf) {
		uint16_t crc = initCRC();
...
private:
	...
	uint8_t handleReceivePackage() {
		...
		wireTimeOut = Timer::millis() + REMOTEXY_TIMEOUT;
#if defined(REMOTEXY_CLOUD)  
